...
class CarStoreServiceImpl @Inject constructor() : CarStoreService {
    override fun getShopByHrAccountAndPosition(hrAccount: String,p2: String, p3: String): Observable<GetShopByHrAccountAndPositionBean> {
        return carStoreRepository.getShopByHrAccountAndPosition(hrAccount,p2,p3).flatMap(Function<GetShopByHrAccountAndPositionBean, ObservableSource<GetShopByHrAccountAndPositionBean>> { t ->
            return@Function Observable.just(t)
        })
    }

    @Inject
    lateinit var carStoreRepository: CarStoreRepository
}