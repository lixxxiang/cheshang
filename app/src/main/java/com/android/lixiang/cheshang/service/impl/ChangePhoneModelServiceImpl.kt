...
import com.android.lixiang.cheshang.presenter.data.repository.ChangePhoneModelRepository
...
class ChangePhoneModelServiceImpl @Inject constructor() : ChangePhoneModelService {
    override fun updateLoginInfo(s1: String, s2: String, s3: String): Observable<UpdateLoginInfoBean> {
        return mChangePhoneModelRepository.updateLoginInfo(s1, s2, s3).flatMap(Function<UpdateLoginInfoBean, ObservableSource<UpdateLoginInfoBean>> { t ->
            return@Function Observable.just(t)
        })
    }

    @Inject
    lateinit var mChangePhoneModelRepository: ChangePhoneModelRepository
}